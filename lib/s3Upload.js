...
const s3Upload = function (xFile) {
  ...
  const stream = fs.createReadStream(xFile.file.path)
  const ext = path.extname(xFile.file.originalname)
  const filename = path.basename(xFile.file.originalname, ext)
  const mimeType = xFile.file.mimetype
  ...
  return promiseRandomBytes()
    .then(rando => {
      return {
        ACL: 'public-read',
        Bucket: 'wb-bitstash-bucket', // OR Bucket: process.env.AWS_BUCKET_NAME
        ContentType: mimeType,
